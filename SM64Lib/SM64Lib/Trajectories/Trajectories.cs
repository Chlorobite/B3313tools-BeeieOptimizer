...
namespace SM64Lib.Trajectorys
{
    public class Trajectories : List<Trajectory>
    {
        ...
        {
            int? addrToSubstract = null;

            if (!checkAddress || (addr > 0x00400000 && addr < 0x00410000))
                addrToSubstract = 0x00400000;
            else if (!checkAddress || ((uint)addr > 0x80400000 && (uint)addr < 0x80410000))
                // Support for old 0x80 addresses
                addrToSubstract = unchecked((int)0x80400000);

            if (addrToSubstract != null)
            {
