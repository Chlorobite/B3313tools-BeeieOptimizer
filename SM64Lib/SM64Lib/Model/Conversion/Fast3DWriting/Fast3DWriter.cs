...
namespace SM64Lib.Model.Conversion.Fast3DWriting
{
    ...
    public class Fast3DWriter
    {
        ...
        /// </summary>
        private void SetOtherMode_H(Material mat, ref bool ciEnabled)
        {
            var citextypes = new[] { N64Codec.CI4, N64Codec.CI8 };
            bool iscitexture = citextypes.Contains(mat.TexType);
            string ciByte;
            string filterByte;
            ...
            // Filter
            switch (mat.TexFilter)
            {
                default:
                case TextureFilter.Point:
                    filterByte = "0";
                    break;
                case TextureFilter.Bilerp:
                    filterByte = "2";
                    break;
                case TextureFilter.Average:
                    filterByte = "3";
                    break;
            }

            // Import command
            ImpF3D($"BA 00 {filterByte}E 02 00 00 {ciByte}0 00");
        }
        ...
        private void ImportObj(Pilz.S3DFileParser.Object3D model)
        {
            ...
            foreach (Fast3DWriting.DisplaylistProps dlProps in dlsToCreate)
            {
                ...
                for (int i = 0, loopTo2 = vertexGroups.Count - 1; i <= loopTo2; i++)
                {
                    ...
                    SetOtherMode_H(mp.Material, ref ciEnabled);
