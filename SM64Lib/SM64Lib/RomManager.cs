...
namespace SM64Lib
{
    public class RomManager
    {
        ...
        public event PrepairingRomEventHandler PrepairingRom;
        public delegate void PrepairingRomEventHandler(RomManager sender, EventArgs e);
        ...
        public RomManager(string FileName, ILevelManager levelManager)
        {
            ...
            CustomAsmAreaConfig.RequestCustomAsmArea += CustomAsmAreaConfig_RequestCustomAsmArea;
        ...
        private void CustomAsmAreaConfig_RequestCustomAsmArea(CustomAsmAreaConfig config, CustomAsmAreaConfig.RequestCustomAsmAreaEventArgs request)
        {
            if (request.CustomAsmArea  == null)
            {
                foreach (var area in GlobalCustomAsmBank.Areas)
                {
                    if (request.CustomAsmArea == null && area.Config == config)
                        request.CustomAsmArea = area;
                }
            }
        }
                    ...
                    foreach (KeyValuePair<string, RomVersion> kvp in dicUpdatePatches.Where(n => n.Value > ProgramVersion).OrderBy(n => n.Key))
                    {
                        if (kvp.Value.Filename.StartsWith("#"))
                        {
                            ...
                            if (patch.ID == paths[1])
                            {
                                ...
                                if (script is object)
                                    mgr.Patch(script, null, dicParams, null);
                            }
                        }
                        else
        ...
        private void PrepairROM()
        {
            ...
            IEnumerable<BaseTweakScriptInfo> allScripts = baseTweak.Scripts
                .Select(n => new BaseTweakScriptInfo(n))
                .OrderBy(n => n.Priority);

            // Raise Event to let the user choose what to enable
            var eargs = new PrepairingRomEventArgs
            {
                ScriptInfos = allScripts
            };
            PrepairingRom?.Invoke(this, eargs);
            ...
            if (allScripts.Any())
            {
                ...
                foreach (var info in allScripts)
                    patchingManager.Patch(info.Script, null, dicParams, null);
            }
