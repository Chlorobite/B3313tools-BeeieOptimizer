...
namespace SM64Lib.Objects.ObjectBanks
{
    public class CustomObjectReferences
    {
        ...
        public IReadOnlyCollection<CustomAsmAreaConfig> ReferenceAsmAreas { get; }
        ...
        private CustomObjectReferences(CustomObject[] refObject, BehaviorConfig[] refBehaviors, CustomAsmAreaConfig[] refAsmAreas, CustomModelConfig[] refModels)
        {
        ...
        public void DeleteReferences(RomManager rommgr)
        {
            ...
            rommgr.GlobalCustomAsmBank.Areas.RemoveRangeIfContains(refAsmAreas.Select(n => n.FindCustomAsmArea()).Where(n => n is object).ToArray());
        ...
        public static CustomObjectReferences Find(CustomObject[] customObjects, RomManager rommgr)
        {
            ...
            var refAsmAreas = new List<CustomAsmAreaConfig>();
            ...
            foreach (var customObject in customObjects)
            {
                if (!customObject.BehaviorProps.UseCustomAddress && customObject.BehaviorProps.Behavior is object)
                {
                    ...
                    if (!refBehaviors.Contains(behavConfig))
                    {
                        ...
                        {
                            if (!refAsmAreas.Contains(asmArea.CustomAsmAreaConfig))
                            {
                                refAsmAreas.Add(asmArea.CustomAsmAreaConfig);
                                FindReferenceBehaviors(rommgr.GlobalBehaviorBank, asmArea.CustomAsmAreaConfig, refBehaviors);
                            }
                        }
                    }
                }
        ...
        private static void FindReferenceBehaviors(BehaviorBank bank, CustomAsmAreaConfig asmArea, List<BehaviorConfig> refBehaviors)
        {
            foreach (var behav in bank.Behaviors)
            {
                foreach (var link in behav.Config.CustomAsmLinks)
                {
                    if (link.CustomAsmAreaConfig == asmArea)
